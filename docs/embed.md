...
On distribution: loading `/embed.js` from your own instance is still the path we recommend — the player version always matches your instance's API, so there is nothing to keep in sync. `@demo-locker/player` **is** published for build-time bundling (see above); this paragraph used to say it was planned, which contradicted the install instructions higher up the same page. The trade is the one you would expect: bundling pins a player version that your instance's API can drift away from, so upgrade them together. If you load the script from a CDN instead, pin an exact version and use Subresource Integrity (SRI) — don't point at a floating `latest`. Note: on the hosted Worker deployment, `/embed.js` is served by Cloudflare's asset layer rather than the Hono route, so headers may differ slightly from a self-hosted instance.
