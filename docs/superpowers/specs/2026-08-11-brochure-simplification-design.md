...
**Budget.** 900px at 2x, about 10 seconds, under 600 KB for WebM + MP4 + poster
combined. If it does not fit, the length comes down before the resolution does.
...
The existing "what to bring with you" list, plus the `docker run` snippet
relocated from the install block, plus the three docs links currently in
`.fineprint`. Someone reading this section has already decided to host it;
Docker belongs to them, not to the first screenful.
