...
**CORRECTION 2026-07-28 — the spike above was incomplete and shipped a bug.** It probed
`/playlists/x` but never bare `/playlists`, and **`/playlists/*` does not match `/playlists`**. With
only the wildcard form, every *collection* endpoint fell through to the assets layer and returned the
SPA index — create-playlist, list-playlists, list-shares and the entire track library were broken on
every fresh Cloudflare install of `demo-locker@0.2.0`, while every sub-path looked perfectly healthy.
It was caught by an agent that actually used the product end to end, not by the spike.

The list now carries **both forms for every prefix** (`/auth` and `/auth/*`, `/playlists` and
`/playlists/*`, …), re-verified against a real Worker using config generated by `buildPlan` itself
rather than hand-written. Fixed in 0.2.1.

**Lesson: when probing a routing pattern, test the bare prefix as well as a sub-path.** A wildcard
that works for one tells you nothing about the other, and the half that fails is the half where
collection endpoints live.
